                             ...
                             bool bClear, formatEndian endianness)
{
    ...
    wxLogDebug("yuvBuffer::alloc_buffer(%d, %d, %d, %d, %s)",
               width, height, bits, (bClear==true)?1:0, (endianness == endian_little)?"le":"be");
    ...
    if (bits == 8) {
        ...
        wxLogDebug("Invalid buffer depth of %d, assuming 1 byte per component", bits);
        bpc = sizeof(wxUint8);
    }
            ...
            (bits != m_bits))
    {
        ...
        m_pY = (wxUint8*)realloc((void*)m_pY, m_lumaSize);
    ...
    if (bClear) {
        ...
        memset(m_pY, 0, m_lumaSize);
        memset(m_pU, 0, m_chromaSize);
        memset(m_pV, 0, m_chromaSize);
    }
...
wxUint8* yuvBuffer::DoConversionToRGB(dataType bufType, bool ccir601) {
    ...
    wxLogDebug("yuvBuffer::DoConversionToRGB(%d, %d)", (int)bufType, (int)ccir601);
    ...
    for (row = 0; row < (rows & ~1); row += 2) {
        ...
        pP1 = (PIXEL *)((unsigned long)imageData + (row * m_yuvWidth * sizeof(PIXEL)));
        pP2 = (PIXEL *)((unsigned long)imageData + ((row + 1) * m_yuvWidth * sizeof(PIXEL)));
...
ImageBuffer::ImageBuffer(dataType type,
...
bool ImageBuffer::SaveYUV(wxString sFilename, eSaveType type) {
    ...
    wxLogDebug("ImageBuffer::SaveYUV(%s, %d)", (const char*)sFilename.c_str(), (int)type);
